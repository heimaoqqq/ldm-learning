...
def clone_taming_transformers():
    """克隆官方taming-transformers项目"""
    print("📥 克隆官方taming-transformers项目...")
    ...
    for path in taming_paths:
        if path.exists() and (path / "taming").exists():
            print(f"✅ 找到已存在的taming-transformers: {path.resolve()}")
            return path.resolve()
    
    # 如果不存在，则克隆
    try:
        # 尝试克隆到上级目录
        clone_path = Path("../taming-transformers")
        print(f"克隆到: {clone_path.resolve()}")
        ...
        try:
            ...
            print(f"❌ 克隆失败: {e2}")
            print("请检查网络连接或手动下载taming-transformers")
            return None
...
def setup_kaggle_taming_environment():
    ...
    # 克隆taming-transformers项目
    taming_dir = clone_taming_transformers()
    if taming_dir is None:
        print("❌ 无法获取taming-transformers")
        return False
    ...
    print(f"📂 taming-transformers位置: {taming_dir}")
    return True, str(taming_dir)

def create_kaggle_custom_dataset(taming_path):
    """创建Kaggle自定义数据集类"""
    dataset_code = f'''
import os
import torch
...
def create_kaggle_training_script(taming_path):
    """创建Kaggle训练脚本"""
...
def main():
    ...
    try:
        ...
        print(f"❌ 导入失败: {{e}}")
        print("请检查taming-transformers路径")
        return
...
def create_evaluation_script(taming_path):
    """创建模型评估脚本"""
...
def main():
    ...
    # 1. 环境设置和克隆项目
    result = setup_kaggle_taming_environment()
    if isinstance(result, tuple):
        success, taming_path = result
        if not success:
            print("❌ 环境设置失败")
            return
    else:
    ...
    # 2. 创建自定义数据集
    create_kaggle_custom_dataset(taming_path)
    
    # 3. 创建P100优化配置
    create_p100_vqgan_config()
    
    # 4. 创建训练脚本
    create_kaggle_training_script(taming_path)
    
    # 5. 创建评估脚本
    create_evaluation_script(taming_path)
    ...
    print("\n💡 关键特性:")
    print("✅ 自动克隆官方taming-transformers")
    ...
    print(f"- taming-transformers路径: {taming_path}")
